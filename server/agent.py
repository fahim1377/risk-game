...
from chanceNode import chance_Node
import sys
...
class Agent:
    ...
    def a_star(self, reinforce_threshold=1,attack_threshold=2):
        ...
        while not frontier.empty():
            ...
            if self.semi_goal_test(node) and timeit.default_timer()-start>1  or timeit.default_timer()-start>2:
                stop = timeit.default_timer()
                return True,stop-start,self.get_path(node)
    ...
    def minimize(self, node, alpha, beta, player, avg_branch_factor):
        node.player=player
        # move = node.prev_action['move_type'] if node.prev_action else None
        # print("\nMIN:","player:", node.player.id,"depth:", node.depth,"territories:", len(node.state[node.player.id].items()),'move:', move,"cost:", node.utility, "avg_branch_factor:", avg_branch_factor)
        if self.terminal_test(node, avg_branch_factor) or self.goal_test(node):
            return None, node.utility
        min_child,min_utility = None,float('inf')
        children =  node.get_neighbors(6,2)
        avg_branch_factor = (node.depth * avg_branch_factor + len(children)) /( node.depth+1)
        for child in children:
            child.calculate_utility()
            if child.prev_action and child.prev_action['move_type']=='end_turn':
                _,utility = self.maximize(child, alpha, beta, avg_branch_factor)
            else:
                _,utility = self.minimize(child, alpha, beta, player, avg_branch_factor)
    ...
    def maximize(self, node, alpha, beta, avg_branch_factor):
        # move = node.prev_action['move_type'] if node.prev_action else None
        # print("\nMAX:","player:", node.player.id,"depth:", node.depth,"territories:", len(node.state[node.player.id].items()),'move:', move,"cost:", node.utility, "avg_branch_factor:", avg_branch_factor)
        if self.terminal_test(node, avg_branch_factor) or self.goal_test(node):
            return None, node.utility
        max_child,max_utility = None,float('-inf')
        utility=0
        children =  node.get_neighbors(4,3.5)
    ...
    def minimax(self,phase=0):
        start = timeit.default_timer()
        root = chance_Node(self.game,self.game.state, self.player,stochastic=self.stochastic,phase=phase)
        root.calculate_utility()
        child,_ = self.maximize(root,float('-inf'),float('inf') , 1)
        prev_action = None
        if child:
            prev_action = child.prev_action
        return True,timeit.default_timer()-start,prev_action
    
    def terminal_test(self,node,avg_branch_factor):
        slow_ness = avg_branch_factor**node.depth
        # print(f'MYLOG : child count : {slow_ness}')
        return (node.depth > 8) or (slow_ness > 200000)